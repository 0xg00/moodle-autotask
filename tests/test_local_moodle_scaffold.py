...
def test_real_no_checkout_clone_allows_only_main_tracking_metadata(tmp_path: Path) -> None:
    ...
    driver.write_text(
        ...
        + f"    RepositoryUrl = '{bare_remote.as_posix()}'\n"
        + "    GitPath = $git\n}\n"
        + "Ensure-GitRepository @cloneArguments\n"
        + f"$remote = & $git -C '{checkout.as_posix()}' config --local --get branch.main.remote\n"
        + f"$merge = & $git -C '{checkout.as_posix()}' config --local --get branch.main.merge\n"
